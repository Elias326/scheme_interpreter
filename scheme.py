...
def do_if_form(expressions, env):
    ...
    if scheme_true(scheme_eval(expressions.first, env)) == True:
...
def do_and_form(expressions, env):
    ...
    while expressions != nil:
        if expressions.second!=nil:
            rv = scheme_eval(expressions.first, env)
        else:
            rv = scheme_eval(expressions.first, env, True)
        if scheme_false(rv):
            return False
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        if not scheme_symbolp(binding.first):
            raise SchemeError
        params = Pair(binding.first, params)
...
def check_formals(formals):
    ...
    parameter_list = []
    while(formals != nil):
        if formals.first not in parameter_list and scheme_symbolp(formals.first):
            parameter_list.append(formals.first)
        else:
            raise SchemeError    
